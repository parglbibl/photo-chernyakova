...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // ===== КНОПКА VK (Прямая ссылка на репост) =====
    document.querySelectorAll('.share-btn').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            if (isMobile) {
                // На телефонах: пытаемся открыть через приложение, если не получится — браузер
                // Используем схему vk://, но только если это не iOS (там она работает хуже)
                if (/iPhone|iPad|iPod/i.test(navigator.userAgent)) {
                    // iOS: используем универсальную ссылку
                    window.location.href = vkShareUrl;
                } else {
                    // Android: пробуем приложение
                    ...
                    // Пытаемся открыть приложение, если не открывается — браузер через 500мс
